...
// lib/php/themes/bootstrap/sshm.php 20230703 - 20250117
...
class Themes_Bootstrap_Sshm extends Themes_Bootstrap_Theme
{
    ...
    private function modalBody(array $in): string
    {
        ...
        $skeyBuf = $this->dropdown($skeyOptions, 'skey', $in['skey'], '', 'form-control');
    ...
    private function generateModalFooter(string $lhsCmd, string $midCmd, string $rhsCmd): string
    {
        ...
        if ($midCmd) $buttons[] = "<button type='button' class='btn btn-info' data-dismiss='modal'>$midCmd</button>";
    ...
    private function generateTableRows(array $rows): string
    {
        ...
        return implode('', array_map(
            fn(string $row): string => $this->generateTableRow(...preg_split('/\s+/', $row)),
            $rows
        ));
    }

    private function generateTableRow(
        string $name,
        string $host,
        string $port,
        string $user,
        ...
        $skeyLink = (!$skey || $skey === 'none')
            ? ''
            : "<a class='bslink' href='?o=sshm&m=key_read&skey=$skey'><b>$skey</b></a>";
        return <<<HTML
        <tr>
    ...
    private function generateListHTML(string $rows): string
    {
        ...
        <div class="row mb-1">
            <div class="d-flex justify-content-between">
                <h3>
                    <i class="fa fa-key"></i> SSH Manager
                    <a href="?o=sshm&m=create" class="bslink" title="Add new SSH Host config">
                        <small><i class="fa fa-plus-circle"></i></small>
                    </a>
                </h3>
                <div>
    ...
    private function generateKeyTableRows(array $in): string
    {
        ...
        if (isset($in['err']) && ($in['err'] === 254 || $in['err'] === 255)) {
            $lvl = $in['err'] === 254 ? 'warning' : 'danger';
            util::log($in['ary'][0], $lvl);
            return '';
        }
        return implode('', array_map(
            fn(string $row): string => $this->generateKeyTableRow(...preg_split('/\s+/', $row)),
            $in['ary'] ?? []
        ));
    }

    private function generateKeyTableRow(
        string $name,
        string $size,
        string $fingerprint,
        string $comment,
    ...
    private function generateModals(): string
    {
        ...
        return implode('', array_map(
            fn(string $modal): string => $this->generateModal($modal),
            $modals
        ));
    }
