...
// lib/php/themes/bootstrap5/records.php 20180714 - 20250121
...
class Themes_Bootstrap5_Records extends Themes_Bootstrap5_Theme
{
    ...
    private function generateHtml(array $in): string
    {
        ...
        $csrfToken = (string)($_SESSION['c'] ?? '');
        $currentObject = (string)($this->g->in['o'] ?? '');
        $domainId = (string)($in['did'] ?? '');
        $domain = htmlspecialchars((string)($in['domain'] ?? ''));
        $typeDropdown = $this->dropdown(
            array_map(
                fn(string $type): array => [$type, $type],
                self::TYPES
            ),
            'type',
            'A',
