<?php declare(strict_types=1);
// lib/php/plugins/mailgraph.php 20170225 - 20250117
// Copyright (C) 2015-2025 Mark Constable <markc@renta.net> (AGPL-3.0)

class Plugins_MailGraph extends Plugin
{
    public function list() : string
    {
        elog(__METHOD__);

        $return = '';
        $images = ['0-n', '1-n', '2-n', '3-n'];
        
        foreach ($images as $image) {
            $imageUrl = 'http://localhost:81/mailgraph.cgi?' . $image;
            $headers = @get_headers($imageUrl);
            
            if ($headers === false) {
                // Connection failed
                $return .= '<div class="alert alert-warning" role="alert">
                    <strong>Service Unavailable:</strong> Could not connect to mailgraph service on port 81. Please ensure mailgraph is running.
                </div>';
                continue;
            }
            
            if (!isset($headers[0])) {
                // Invalid response
                $return .= '<div class="alert alert-danger" role="alert">
                    <strong>Error:</strong> Invalid response from mailgraph service.
                </div>';
                continue;
            }
            
            $return_code = (int)substr($headers[0], 9, 3);
            if ($return_code >= 400) {
                $return .= '<img class="img-responsive" alt="not-yet-available" src="data:image/png;base64," /><p>Not enough data</p>';
            } else {
                $imageData = @file_get_contents($imageUrl);
                if ($imageData === false) {
                    $return .= '<div class="alert alert-danger" role="alert">
                        <strong>Error:</strong> Failed to fetch graph data from mailgraph service.
                    </div>';
                    continue;
                }
                $return .= '<img class="img-responsive" alt="' . htmlspecialchars($imageUrl) . '" src="data:image/png;base64,' . base64_encode($imageData) . '" />';
            }
        }
        return $this->t->list(['mailgraph' => $return]);
    }
}

?>
