...
// lib/php/util.php 20150225 - 20250116
...
class Util
{
     ...
     * @param string $v Default value if key doesn't exist
     * @param string|null $x New value to set if different from current
     */
    ...
    public static function cfg(Config $g): void
    {
        elog(__METHOD__);
    ...
    public static function run(string $cmd): string
    {
        ...
        return (string)exec('sudo ' . escapeshellcmd($cmd) . ' 2>&1');
    }
    ...
    public static function now(string $date1, ?string $date2 = null): string
    {
        ...
        if (!is_numeric($date1)) {
            $date1 = (string)strtotime($date1);
        }
        if ($date2 && !is_numeric($date2)) {
            $date2 = (string)strtotime($date2);
        }
        $date2 = $date2 ?? (string)time();
        $diff = abs((int)$date1 - (int)$date2);
    ...
    /**
     * @param array<string, array<array<string>>> $nav Navigation configuration
                ...
                if (preg_match('/[A-Z]+/', $pw)) {
                    if (preg_match('/[a-z]+/', $pw)) {
                        if ($pw2) {
                            if ($pw === $pw2) {
                                return true;
                            } else self::log('Passwords do not match, please try again');
                        } else return true;
                    } else self::log('Password must contains at least one lower case letter');
                } else self::log('Password must contains at least one captital letter');
            } else self::log('Password must contains at least one number');
        } else self::log('Passwords must be at least 12 characters');
        return false;
    }

    public static function chkapi(Config $g): void
    {
        elog(__METHOD__);
    ...
    public static function remember(Config $g): void
    {
        elog(__METHOD__);
...
<pre style="width:50em;margin:0 auto;">' . $msg . '</pre>';
    ...
    public static function numfmt(float $size, ?int $precision = null): string
    {
        ...
        elseif ($size >= 1000000000) return round(($size / 1000000000), $precision ?? 2) . ' GB';
        elseif ($size >= 1000000) return round(($size / 1000000), $precision ?? 1) . ' MB';
        elseif ($size >= 1000) return round(($size / 1000), $precision ?? 0) . ' KB';
